function couple_script_rot_cells(paramfile)
%% Import parameters
fid = fopen(paramfile);
tline = fgetl(fid);
while ischar(tline)
...
%  rotate around Y --------------------------------------------------------
radiansY = deg2rad(yangle); % convert to radians
Ry = [cos(radiansY), 0, sin(radiansY);
      0, 1, 0;
      -sin(radiansY), 0, cos(radiansY)];
rotmat = [R * Ry, nrnloc2'; 0 0 0 1];

% rotate around Z ---------------------------------------------------------
% angleZ = 180; % angle of rotation around Y axis, in degrees
% radiansZ = deg2rad(angleZ); % convert to radians
% Rz = [cos(radiansZ) -sin(radiansZ) 0;
%      sin(radiansZ) cos(radiansZ) 0;
%      0 0 1];
% 
% rotmat = [R * Rz, nrnloc2'; 0 0 0 1];
% -------------------------------------------------------------------------
...
fprintf(fid, ['yangle = ' num2str(yangle) ';\n']);
fclose(fid);
