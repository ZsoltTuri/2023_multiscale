...
% Neuronal model: Aberra_23
% Neuronal morphology: Aberray L5

function[] = Aberra_L23b_model_script(params)
fclose('all');
...
dist_input{1} = num2str(params.syn_distance);
...
for t                    = 1 : numel (tree)
    ...
    neuron.mech{t}.axon.CaDynamics_E2 = struct(...
        'gamma', 0.016713,...
        'decay', 384.114655);
    neuron.mech{t}.axon.Nap_Et2 = struct('gNap_Et2bar', 0.009803);
    neuron.mech{t}.axon.SK_E2 = struct('gSK_E2bar', 0.008085);
    neuron.mech{t}.axon.Ca_HVA = struct('gCa_HVAbar', 0.000306);
    neuron.mech{t}.axon.K_Pst = struct('gK_Pstbar', 0.959296);
    neuron.mech{t}.axon.SKv3_1 = struct('gSKv3_1bar', 0.094971);
    neuron.mech{t}.axon.Ca_LVAst = struct('gCa_LVAstbar', 0.000050);
    
    neuron.mech{t}.soma.CaDynamics_E2 = struct(...
            'gamma',  0.000533, ...
            'decay', 342.544232);
    neuron.mech{t}.soma.SKv3_1 = struct('gSKv3_1bar', 0.102517);
    neuron.mech{t}.soma.SK_E2 = struct('gSK_E2bar', 0.099433);
    neuron.mech{t}.soma.Ca_HVA = struct('gCa_HVAbar', 0.000374);
    neuron.mech{t}.soma.NaTs2_t = struct('gNaTs2_tbar', 0.926705);
    neuron.mech{t}.soma.Ih = struct('gIhbar', 0.000080);
    neuron.mech{t}.soma.Ca_LVAst = struct('gCa_LVAstbar', 0.000778);
