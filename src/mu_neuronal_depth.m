function mu_neuronal_depth(index)
    %% Info
    % Run multi-scale modeling on HPC.
    % Aim: Study effect of neuronal depth.
    ...
    params.mesh_model = params.mesh_models{35, 1};
    ...
    params.neuronal_depths = [1.25; 2.25; 3.25];
    ...
    params.sim_id = strcat(params.folder_flag, '_neuronal_depth');
    ...
    if index == 1
        index = randsample(1:length(sim_params.nrn_locations_all), sim_params.nrn_number);
        save(fullfile(params.folder, 'index.mat'), 'index');
    ...
    save(fullfile(params.folder, strcat(sim_params.label, '_depth_', num2str(sim_params.neuronal_dept), '_sim_params.mat')), 'sim_params');
