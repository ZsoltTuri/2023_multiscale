function mu_rotate_cell_y_axis(index)
    %% Info
    ...
    params.angles = transpose(0); % coil angles  
    params.yangles = transpose(0:30:330); % rotation angle of the neuron w.r.t. the y-axis  
    ...
    params.sim_id = strcat(params.folder_flag, '_rotate_cell');
    ...
    sim_params.efield_sim = strcat(params.mesh_model, '_angle_', num2str(params.angles)); 
    ...
    sim_params.yangle = params.yangles(index, 1); 
    if index == 1
        copy_mesh(params, sim_params);
    end    
    sim_params.label = params.sim_id;
    sim_params.rot_cells = 'yes';
