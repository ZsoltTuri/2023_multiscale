...
d <- mu_neuronal_model %>% dplyr::count(neuronal_model)
d$activated_cells <- mu_neuronal_model %>% dplyr::filter(activation_threshold < 101) %>% dplyr::count(neuronal_model) %>% dplyr::pull(n)
d$percent <- 100 * round(d$activated_cells/d$n, 4)
d
d$percent[2]/d$percent[1]
...
ggsave(paste("graphs/figure_2/A.", format, sep = ""), plot = last_plot(), 
       width = 60, height = 65, units = "mm", dpi = 300)

# GLM with binomial distribution
data <- mu_neuronal_model %>% dplyr::mutate(neuronal_model = factor(neuronal_model),
                                            activated = case_when(activation_threshold < 101 ~ 1,
                                                                  activation_threshold == 101 ~ 0,
                                                                  TRUE ~ NA_real_))
data <- within(data, 
               neuronal_model <- relevel(neuronal_model, ref = "L23"))
...
ggplot(data =  mu_neuronal_model %>% dplyr::filter(activation_threshold < 101), aes(x = neuronal_model, y = activation_threshold, color = neuronal_model, fill = neuronal_model)) + 
  ...
  theme(axis.text = element_text(colour = "black", size = 12),
        ...
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        legend.position = "none")
ggsave(paste("graphs/figure_2/B.", format, sep = ""), plot = last_plot(), 
       width = 60, height = 65, units = "mm", dpi = 300)
mu_neuronal_model %>% dplyr::filter(activation_threshold < 101) %>% dplyr::group_by(neuronal_model) %>% dplyr::summarise_at(vars(activation_threshold), list(mean = mean, sd = sd)) %>% dplyr::ungroup()
...
df = data.frame('DV' = mu_neuronal_model$activation_threshold,
                'IV' = mu_neuronal_model$neuronal_model)
...
mu_neuronal_model_actefield %>% dplyr::filter(activated == TRUE) %>% dplyr::group_by(neuronal_model) %>% dplyr::count(neuronal_model) %>% dplyr::ungroup()
ggplot(data =  mu_neuronal_model_actefield %>% dplyr::filter(activated == TRUE), aes(x = neuronal_model, y = activation_efield, color = neuronal_model, fill = neuronal_model)) + 
  geom_jitter(alpha = 0.25) +
  stat_summary(fun.data = "mean_cl_boot", colour = "white", linewidth = 0.5, size = 0.05) +
  ...
  theme(axis.text = element_text(colour = "black", size = 12),
        ...
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        legend.position = "none")
ggsave(paste("graphs/figure_2/C.", format, sep = ""), plot = last_plot(), 
       width = 60, height = 65, units = "mm", dpi = 300)
...
mu_neuronal_model_actefield %>% dplyr::filter(activated == TRUE) %>% dplyr::group_by(neuronal_model) %>% dplyr::summarise_at(vars(activation_efield), list(mean = mean, sd = sd)) %>% dplyr::ungroup()
